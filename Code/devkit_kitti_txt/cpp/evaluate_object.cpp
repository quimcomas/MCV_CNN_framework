...
void initGlobals () {
  ...
  CLASS_NAMES_CAP.push_back("Van");
  CLASS_NAMES_CAP.push_back("Truck");
  CLASS_NAMES_CAP.push_back("Person sitting");
  CLASS_NAMES_CAP.push_back("Tram");
}
...
vector<tDetection> loadDetections(string file_name, bool &compute_aos,
  ...
  while (!feof(fp)) {
    ...
    if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                   ...
                   &d.ry, &d.thresh)==16) {
    // if (fscanf(fp, "%s %lf %lf %s %lf %lf %lf %lf %s %s %s %s %s %s %s %lf",
    //            str, &trash, &trash, &str_trash, &d.box.x1, &d.box.y1,
    //            &d.box.x2, &d.box.y2, &str_trash, &str_trash, &str_trash, &str_trash, &str_trash, &str_trash,
    //            &str_trash, &d.thresh)==16) {
      ...
      if(d.box.alpha == -10 || d.box.alpha == -10)
        compute_aos = false;
...
bool eval(string result_sha, string gt_sha, string path_out, Mail* mail){

  // set some global parameters
  initGlobals();

  // ground truth and result directories
//  string gt_dir         = "/datatmp/Datasets/detection/bdd100k/labels_kitti_cut/100k/val";
  string result_dir     = result_sha;
  string plot_dir       = path_out + "/plot";
  ...
  vector<std::string> vNames(0);
//  DIR *dir;
//  struct dirent *ent;
//  if ((dir = opendir ((result_dir + "/data").c_str())) != NULL)
//  {
//      int cont = 0;
//      while ((ent = readdir (dir)) != NULL)
//      {
//        if (cont>1)
//            vNames.push_back(ent->d_name);
//        cont++;
//      }
//      closedir (dir);
//  }
...
//  std::cout << "exiting debugging file..." << gt_sha << std::endl;
  std::ifstream infile(gt_sha.c_str());
  string read_line;
  while (infile >> read_line) vNames.push_back(read_line);

//  std::cout << vNames.size() << std::endl;
//  std::cout << vNames[0] << std::endl;
//
//  exit(-1);
//  std::cout << "did it exit?" << std::endl;



  int N_TESTIMAGES = vNames.size();
...
//  mail->msg("Loading detections...");
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    full_name = vNames[i];
//    full_name = full_name.substr(0,full_name.find("."))+".txt";
//    string inv_full_name = full_name.reverse(full_name.begin(),full_name.end());

//    std::cout << "full name: " << full_name << std::endl;

    std::reverse(full_name.begin(),full_name.end());
    file_name = full_name.substr(0,full_name.find("/"));
    std::reverse(file_name.begin(),file_name.end());
    std::reverse(full_name.begin(),full_name.end());


//    std::cout << "reverse full name: " << full_name << std::endl;
//   std::cout << "file name: " << file_name << std::endl;
//    exit(-1);

    // read ground truth and result poses
    bool gt_success,det_success;
    vector<tGroundtruth> gt   = loadGroundtruth(full_name,gt_success);
    ...
    // check for errors
    if (!gt_success) {
      mail->msg("ERROR Eval: Couldn't read: %s of ground truth. Please write me an email!", full_name.c_str());
      return false;
    }
    if (!det_success) {
    string det_file = result_dir + "/data/" + file_name;
      mail->msg("ERROR Eval: Couldn't read: %s", det_file.c_str());
      return false;
    }
  }
//  mail->msg("  done.");
...
//    std::cout << "loop for each class " << c << "/" << NUM_CLASS << std::endl;
    if (eval_image[c]) {
//      std::cout << "evalued" << std::endl;
//      mail->msg("Starting 2D evaluation (%s) ...", CLASS_NAMES[c].c_str());
      fp_det = fopen((path_out + "/stats_" + CLASS_NAMES[c] + "_detection.txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((path_out + "/stats_" + CLASS_NAMES[c] + "_orientation.txt").c_str(),"w");
...
/*  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_ground[c]) {
      ...
      fp_det = fopen((path_out + "/stats_" + CLASS_NAMES[c] + "_detection_ground.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
...
*/
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_3d[c]) {
      ...
      fp_det = fopen((path_out + "/stats_" + CLASS_NAMES[c] + "_detection_3d.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=3 && argc!=6 && argc!=4) {
    cout << "Usage: ./eval_detection detection_path txt_gt [path_out] [user_sha email]" << endl;
    cout << "send " << argc << " arguments:" << "\n\t";
  ...
  string gt_sha = argv[2];

  // init notification mail
  Mail *mail;
  if (argc==6) mail = new Mail(argv[5]);
  else         mail = new Mail();
  ...
  // run evaluation
  if (eval(result_sha,gt_sha,path_out,mail)) {
    //mail->msg("Your evaluation results are available at:");
    //mail->msg("http://www.cvlibs.net/datasets/kitti/user_submit_check_login.php?benchmark=object&user=%s&result=%s",argv[4], result_sha.c_str());
  } else {
