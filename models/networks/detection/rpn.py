...
class RPN(Net):
    ...
    def __init__(self, cf, din):
        ...
        self.nc_score_out = len(self.cf.anchor_scales) * len(self.cf.anchor_ratios) * 2 # 2(bg/fg) * 9 (anchors)
        self.RPN_cls_score = nn.Conv2d(512, self.nc_score_out, 1, 1, 0)

        # define anchor box offset prediction layer
        self.nc_bbox_out = len(self.cf.anchor_scales) * len(self.cf.anchor_ratios) * 4 # 4(coords) * 9 (anchors)
        self.RPN_bbox_pred = nn.Conv2d(512, self.nc_bbox_out, 1, 1, 0)

        # define proposal layer
        self.RPN_proposal = ProposalLayer(self.cf)
