...
class Classifier_Module(nn.Module):
    def __init__(self, dilation_series, padding_series, NoLabels):
        ...
        for dilation,padding in zip(dilation_series, padding_series):
            self.conv2d_list.append(nn.Conv2d(2048, NoLabels, kernel_size=3, stride=1, padding=padding,
...
class MS_Deeplab(Net):
    def __init__(self, cf, n_classes=21, pretrained=False, net_name='DeepLabv2'):
        super(MS_Deeplab, self).__init__(cf)
        self.url = 'http://datasets.cvc.uab.es/models/pytorch/basic_fcn8.pth'
        self.pretrained = pretrained
        self.net_name = net_name
        self.Scale = ResNet(Bottleneck, [3, 4, 23, 3], n_classes)   #changed to fix #4
